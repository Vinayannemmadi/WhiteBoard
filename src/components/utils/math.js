...
export const getArrowHeadsCoordinates=(x1,y1,x2,y2)=>{
    ...
    const x3=x2-length* Math.cos(angle- Math.PI/6);
    const y3=y2-length* Math.sin(angle- Math.PI/6);
    
    const x4=x2-length* Math.cos(angle- Math.PI/6);
    const y4=y2-length* Math.sin(angle- Math.PI/6);
    // console.log("x3 \n"+x3,"y3 "+y3,"x4 "+x4,"y4 "+y4);
    return {
        x3,
        y3,
        x4,
